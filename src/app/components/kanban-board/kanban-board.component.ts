  ...
  animations: [
    ...
    trigger("columnExpand", [
      ...
      state(
        "collapsed",
        style({
          height: "120px", // Ajustado para acomodar o indicador de contagem
          opacity: 1,
        })
      ),
...
export class KanbanBoardComponent implements OnInit {
  ...
  columnStates: { [key: string]: "expanded" | "collapsed" } = {
    todo: "expanded",
    inProgress: "expanded",
    done: "expanded",
  };

  // Função refatorada para alternar o estado de uma única coluna
  toggleColumn(column: "todo" | "inProgress" | "done") {
    const newState =
      this.columnStates[column] === "expanded" ? "collapsed" : "expanded";
    this.columnStates[column] = newState;
    this.saveColumnStates(); // Salva o estado no localStorage
  }

  // Nova função para expandir/colapsar todas as colunas
  toggleAllColumns(expand: boolean) {
    const state = expand ? "expanded" : "collapsed";
    this.columnStates = {
      todo: state,
      inProgress: state,
      done: state,
  ...
  // Função para persistir os estados das colunas no localStorage
  private saveColumnStates() {
    localStorage.setItem(
      "kanbanColumnStates",
      JSON.stringify(this.columnStates)
    );
  }

  // Função para obter o número de tarefas em uma coluna
  getTaskCount(column: "todo" | "inProgress" | "done"): number {
    switch (column) {
      case "todo":
  ...
  ngOnInit() {
    this.route.params.subscribe((params) => {
      ...
      // Carrega os estados das colunas do localStorage
      const savedStates = localStorage.getItem("kanbanColumnStates");
      if (savedStates) {
        try {
          this.columnStates = JSON.parse(savedStates);
        } catch (e) {
          console.error("Erro ao carregar estados das colunas:", e);
        }
      }
    });
  }
  ...
  addTag() {
    ...
    if (newTag && !this.newTask.tags?.includes(newTag)) {
      this.newTask.tags = this.newTask.tags
        ? [...this.newTask.tags, newTag]
        : [newTag];
    }
    this.tag = "";
  }
  ...
  removeTag(tagToRemove: string) {
    if (this.newTask.tags) {
      this.newTask.tags = this.newTask.tags.filter(
        (tag) => tag !== tagToRemove
      );
    }
  }
  ...
  saveEdit() {
    ...
    switch (updatedTask.status) {
      case "todo":
        ...
        break;
      case "inProgress":
        this.inProgress = this.inProgress.map((t) =>
          t.id === updatedTask.id ? updatedTask : t
        );
        break;
      case "done":
        this.done = this.done.map((t) =>
          t.id === updatedTask.id ? updatedTask : t
        );
        break;
      default:
  ...
  addEditTag() {
    ...
    if (newTag && !this.editingTask.tags?.includes(newTag)) {
      this.editingTask.tags = this.editingTask.tags
        ? [...this.editingTask.tags, newTag]
        : [newTag];
    }
    this.editTag = "";
  }
  ...
  removeEditTag(tagToRemove: string) {
    if (this.editingTask.tags) {
      this.editingTask.tags = this.editingTask.tags.filter(
        (tag) => tag !== tagToRemove
      );
    }
  }
